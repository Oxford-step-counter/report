...
for (Point candidate in points) {
    ...
    \chapter{Results}
        ...
        \section{Variability in Surface and Phone}
            ...
            The first thing to do with the set of results is to look at the differences between floor surfaces and phone devices. It is not expected that the device will have any impact as the accelerometers should record the same data. It is possible that the surface may have an impact. 

            \subsection{Surface}

                There are 4 recordings from the Google Pixel on each of the surfaces listed: hard (stone or linoleum) and carpet. All of these recordings were done with the phone in hand. To check the variation between these surfaces, the algorithm parameters will be held constant and the average score will be checked for each surface. The results of these can be seen below in Table \ref{tbl_surface_results}. Note that the parameters used were those of the optimal set, detailed in the next section.
                ...
                If we assume that the distribution of the scores will be normal, we can construct a new distribution with the two surfaces such that:
            ...
            \subsection{Phone}
                ...
                The difference between phones is expected to be effectively zero. In order to prove this, 2 data recordings were taken with each: Google Pixel, Google Nexus 5, and Samsung S6. The phone was held in hand and the surface was a hard floor. Similarly to the analysis of the differences of surfaces, the algorithm parameters were locked at the optimal set as described below. The table of results is shown below in Table \ref{tbl_phone_results}
